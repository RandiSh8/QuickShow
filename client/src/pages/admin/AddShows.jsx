...
const AddShows = () => {
  ...
  try {
    const {data} = await axios.get("/api/show/now-playing",{headers:{Authorization:
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast.error('Failed to add show. Try again');
    } finally {
      setAddingShow(false);
    }
  };
                  ...
                  {times.map((time) => (
                    ...
                    >
                      ...
                      <DeleteIcon
                        ...
                        width={15}
                        className="ml-2 text-red-500 hover:text-red-700 cursor-ppointer"
                      />
                    </div>
                  ))}
                </div>
